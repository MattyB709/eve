...
fn teleop() {
    ...
    loop {
        for id in 1..7 {
            let packet_teach = servo_teach.read(id, Sts3215MemoryTableRegister::CURRENT_LOCATION);
            ...
            let packet = servo_follow.write(
                id,
                Sts3215MemoryTableRegister::TARGET_LOCATION,
...
fn main() {
    ...
    loop {
            if let Ok(packet) = servo.read(4, Sts3215MemoryTableRegister::CURRENT_LOCATION) {
                if let Sts3215ResponsePacket::ReadResponse {id, working_condition, data} = packet {
